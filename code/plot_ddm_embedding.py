...
Answers the question "what do the diffusion axes physically mean?" by:
  ...
  2. coloured scatters  — the leading 2-D plane (DC1 vs DC2), one panel per physical
     quantity (body yaw/pitch/roll accel, wing-angle amplitudes), so you can see
     which physical variable each axis organises by;
  3. multi-pair view    — the single most embedding-aligned quantity shown across the
     three leading coordinate pairs (DC1-DC2, DC1-DC3, DC2-DC3);
  4. correlation map    — |corr(coordinate, physical quantity)| as a heatmap + a
     printed table, the numeric version of (2).

Outputs are PNG (matplotlib Agg). The physical quantities are read from the
`color_*` arrays the DDM run stored in the .npz (see default_color_features).

Run from project root:
    python code/plot_ddm_embedding.py
...
def visualize(embedding_path: str, save_dir: str, max_points: int = 20000) -> None:
...
def main() -> None:
    ...
    visualize(args.embedding, save_dir, max_points=args.max_points)
